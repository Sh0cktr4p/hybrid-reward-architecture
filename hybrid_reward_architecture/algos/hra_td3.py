...
class HRATD3(TD3):
    ...
    def train(self, gradient_steps: int, batch_size: int = 100) -> None:
        ...
        for _ in range(gradient_steps):
            ...
            # Compute critic loss: mean over samples, sum over q values
            critic_loss = th.sum(th.mean((current_q_values - target_q_values)**2, dim=0))
...
class TDRewardHRATD3(HRATD3):
    def __init__(
        ...
        super().__init__(
            policy=policy,
            env=env,
            n_reward_signals=n_reward_signals + 1,
            learning_rate=learning_rate,
            buffer_size=buffer_size,
            learning_starts=learning_starts,
            batch_size=batch_size,
    ...
    def train(self, gradient_steps: int, batch_size: int = 100) -> None:
        ...
        for _ in range(gradient_steps):
            ...
            with th.no_grad():
                ...
                # Reshape to compute the min per reward signal
                # Shape: (batch_size, n_reward_signals, n_critics)
                next_q_values = next_q_values.reshape(next_q_values.shape[0], self.n_reward_signals, -1)

                next_q_values, _ = th.min(next_q_values, dim=-1, keepdim=True)
                target_q_values = replay_data.rewards + (1 - replay_data.dones) * self.gamma * next_q_values[:, :-1, :]
            ...
            with th.no_grad():
                td_reward = th.sum((current_q_values[:, :-1, :] - target_q_values) ** 2, axis=(1, 2)).reshape(-1, 1, 1)
                target_td_q_value = td_reward + (1 - replay_data.dones) * self.gamma * next_q_values[:, -1, :]
                target_q_values = th.cat([target_q_values, target_td_q_value], dim=1)
